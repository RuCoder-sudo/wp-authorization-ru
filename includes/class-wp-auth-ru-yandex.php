 ...
 *  2. Редирект на /wp-json/wp-auth-ru/v1/yandex/start (REST endpoint).
 *  3. Плагин перенаправляет на oauth.yandex.ru/authorize?...
 *  4. Яндекс возвращает пользователя на /wp-json/wp-auth-ru/v1/yandex/callback
 *  5. Плагин обменивает code на token, получает профиль, логинит пользователя.
 *
 * ── Redirect URI для Яндекс ──────────────────────────────────────────────────
...
class WP_Auth_Ru_Yandex {
    ...
    private function __construct() {
        ...
        // REST API регистрируем всегда (нужно для callback даже до проверки настроек)
        add_action( 'rest_api_init', array( $this, 'register_rest_routes' ) );

        if ( empty( $this->settings['yandex_enabled'] ) ) {
            return;
        }
    ...
    public function register_rest_routes() {
        // Старт авторизации: кнопка ведёт сюда → редирект на Яндекс
        ...
        // Callback от Яндекс: Яндекс перенаправляет сюда с ?code=...
    ...
    public function rest_start( WP_REST_Request $request ) {
        ...
        if ( $redirect_to ) {
            set_transient( 'wp_auth_ru_yandex_redir_' . $state, esc_url_raw( urldecode( $redirect_to ) ), 10 * MINUTE_IN_SECONDS );
        }
    ...
    // ─── REST: callback от Яндекс ─────────────────────────────────────────────

    public function rest_callback( WP_REST_Request $request ) {
        // Ошибка от Яндекс
        $error = $request->get_param( 'error' );
        if ( $error ) {
            wp_safe_redirect( add_query_arg( 'yandex_oauth_error', urlencode( sanitize_text_field( $error ) ), wp_login_url() ) );
            exit;
        }
    ...
    private function exchange_code_for_token( $code ) {
        ...
        if ( is_wp_error( $response ) ) {
            return new WP_Error( 'yandex_token_request', 'Ошибка запроса токена: ' . $response->get_error_message() );
        }

        $body = json_decode( wp_remote_retrieve_body( $response ), true );

        if ( ! empty( $body['error'] ) ) {
            return new WP_Error( 'yandex_token_error', 'Яндекс: ' . ( $body['error_description'] ?? $body['error'] ) );
        }

        if ( empty( $body['access_token'] ) ) {
            return new WP_Error( 'yandex_no_token', 'Яндекс не вернул access_token.' );
        }
    ...
    private function get_user_profile( $access_token ) {
        ...
        if ( is_wp_error( $response ) ) {
            return new WP_Error( 'yandex_profile_request', 'Ошибка запроса профиля: ' . $response->get_error_message() );
        }

        $body = json_decode( wp_remote_retrieve_body( $response ), true );

        if ( empty( $body ) || ! is_array( $body ) ) {
            return new WP_Error( 'yandex_profile_empty', 'Не удалось получить данные профиля от Яндекс.' );
        }
    ...
    private function login_or_create_user( $profile ) {
        ...
        if ( empty( $email ) ) {
            return new WP_Error( 'yandex_no_email', 'Яндекс не предоставил email. Разрешите доступ к email в настройках OAuth-приложения.' );
        }
    ...
    public function render_button_wp() {
        ...
        <div class="wp-auth-ru-wrap">
            <a href="<?php echo esc_url( $url ); ?>" class="wp-auth-ru-btn wp-auth-ru-btn--yandex">
                <?php echo $this->yandex_icon(); ?>
                Войти через Яндекс
            </a>
    ...
    private function yandex_icon() {
        return '<svg class="wp-auth-ru-icon" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M13.8 6H12.12C10.08 6 8.88 7.08 8.88 8.88C8.88 10.44 9.6 11.4 11.04 12.36L12.12 13.08L8.76 18H11.04L14.16 13.44L13.2 12.84C12.0 12.0 11.28 11.28 11.28 8.76C11.28 8.04 11.64 7.44 12.48 7.44H13.8V6Z" fill="white"/>
            <path d="M13.8 6H15.12V18H13.8V6Z" fill="white"/>
        </svg>';
    }
    ...
    public function show_error( $message ) {
        if ( ! empty( $_GET['yandex_oauth_error'] ) ) {
            ...
            $message .= '<p class="message" style="color:#d63638;background:#fff3f3;padding:8px 12px;border-radius:4px;margin-top:8px;">'
                . '<strong>Ошибка входа через Яндекс:</strong> ' . esc_html( $err ) . '</p>';
        }
        return $message;
    }
...
.wp-auth-ru-checkout-banner {
    ...
    border: 1px solid #ffe0d0; border-radius: 12px; background: #fff8f6;
}
